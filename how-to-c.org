...
I generally recommend using FSM for array whose size will be forever static.
...
This tends to lead to interfaces that are easier to use.x
...
Used either for type punning or as an optimization to save on space.

Kernel uses it in its mega-structs to save on space.

It's pretty hell to read though.

* Structs - That's about it

Nothing too earth shattering or ground-breaking for structs.

Only like 2 undefined behaviours to keep an eye out for.
